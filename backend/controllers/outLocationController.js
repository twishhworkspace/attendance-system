...
const { getAttendanceDayStart, getAttendanceDayEnd } = require('../utils/timezone');
...
const processRequest = async (req, res) => {
  ...
  try {
    ...
    const attendanceDayStart = getAttendanceDayStart(request.createdAt);
    const attendanceDayEnd = getAttendanceDayEnd(request.createdAt);

    const existingAttendance = await prisma.attendance.findFirst({
      where: {
        userId: request.userId,
        checkIn: { gte: attendanceDayStart, lt: attendanceDayEnd }
      }
    });
