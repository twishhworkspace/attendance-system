...
const MasterAdminView = ({ currentView, setGlobalView }) => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [statsRes, compRes, ticketRes, broadcastRes] = await Promise.allSettled([
            ...
            else console.error('Failed to fetch companies:', compRes.reason);
            
            if (ticketRes.status === 'fulfilled') setTickets(ticketRes.value.data);
            else console.error('Failed to fetch tickets:', ticketRes.reason);
            
            if (broadcastRes.status === 'fulfilled') setBroadcasts(broadcastRes.value.data);
            else console.error('Failed to fetch broadcasts:', broadcastRes.reason);
        } catch (err) {
                            ...
                            {tickets.map(ticket => (
                                <div key={ticket.id} className="glass-panel p-6 space-y-4 hover:border-rose-500/30 transition-all cursor-pointer">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <span className="text-[9px] font-mono text-slate-700">{new Date(ticket.createdAt).toLocaleDateString('en-GB')}</span>
                                    </div>
